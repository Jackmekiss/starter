...
| Gateway | Abstract data access contract for a bounded context. | `.agents/skills/domain-layer/references/gateways.md` | Current examples: `AuthBaseQuery`, `SubscriptionBaseQuery`. |
| Adapter | Concrete implementation of a gateway. | `.agents/skills/domain-layer/references/adapters.md` | Current examples: in-memory, fake, RevenueCat subscription adapter. |
| Use-case | Explicit application action modeled under a bounded context. | `.agents/skills/domain-layer/references/use-cases.md`, `core/*/use-cases` | Names should be verb-based and business-first. |
| Bounded context | Folder under `core/` that owns durable product truth. | `.agents/skills/architecture/references/repo-structure.md` | Current contexts: `auth`, `subscription`; `shared` has shared adapters/helpers. |
