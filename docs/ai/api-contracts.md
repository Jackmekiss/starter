...
- Gateway `handle()` methods adapt internal request objects to abstract methods and return `{ data }`.
- Auth mutating flows update auth state in `onQueryStarted`.
- Subscription flows update entitlement, offerings, or error message in `onQueryStarted`.
- Error responses are currently domain-level result unions, not HTTP status objects.
...
- RevenueCat unavailable states return successful transport responses with failure result objects.
