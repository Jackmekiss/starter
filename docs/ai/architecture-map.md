...
| `src/components/ui/`       | Reusable UI primitives and co-located Storybook stories.                                                                | Contains all 19 Fifteen-derived families implemented with local shadcn-style React Native APIs; Gluestack is absent.                                                                                |
...
- Storybook uses official entry-point swapping through `.rnstorybook/index.tsx`; it has no Expo Router route and no core/runtime providers.
